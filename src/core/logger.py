...
class CoreStructuredFormatter(logging.Formatter):
    def format(self, record: logging.LogRecord) -> str:
        ...
        if hasattr(record, "experiment_id"):
            parts["experiment_id"] = record.experiment_id
...
def setup_core_logging(
    ...
    handler.setFormatter(CoreStructuredFormatter())
    logger.addHandler(handler)

    return logger


def get_core_logger(name: Optional[str] = None) -> logging.Logger:
